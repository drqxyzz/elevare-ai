                            ...
                            <AccordionItem key={item.id} value={`item-${item.id}`} className="border rounded-lg bg-card px-4">
                                ...
                                <AccordionContent className="pt-4 pb-6 border-t">
                                    <Tabs defaultValue="youtube" className="w-full">
                                        ...
                                        {/* Content Display Logic (Simplified for History) */}
                                        <TabsContent value="youtube" className="space-y-4">
                                            <div className="space-y-2">
                                                <h3 className="font-semibold text-sm">Titles</h3>
                                                {item.titles?.map((title, i) => (
                                                    <div key={i} className="flex items-center justify-between p-2 bg-muted/50 rounded text-sm">
                                                        <span>{title}</span>
                                                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleCopy(title)}>
                                        ...
                                        <TabsContent value="tiktok" className="space-y-4">
                                            <div className="space-y-2">
                                                ...
                                                {item.headlines?.map((headline, i) => (
                                                    <div key={i} className="flex items-center justify-between p-2 bg-muted/50 rounded text-sm">
                                                        <span>{headline}</span>
                                                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleCopy(headline)}>
                                                            <Copy className="h-3 w-3" />
                                                        </Button>
                                                    </div>
                                                ))}
                                        ...
                                        <TabsContent value="linkedin" className="space-y-4">
                                            <div className="space-y-2">
                                                <h3 className="font-semibold text-sm">Headlines</h3>
                                                {item.headlines?.map((headline, i) => (
                                                    <div key={i} className="flex items-center justify-between p-2 bg-muted/50 rounded text-sm">
                                                        <span>{headline}</span>
                                                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleCopy(headline)}>
                                        ...
                                        <TabsContent value="twitter" className="space-y-4">
                                            <div className="space-y-2">
                                                <h3 className="font-semibold text-sm">Tweets</h3>
                                                {item.headlines?.map((headline, i) => (
                                                    <div key={i} className="flex items-center justify-between p-2 bg-muted/50 rounded text-sm">
                                                        <span>{headline}</span>
                                                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleCopy(headline)}>
